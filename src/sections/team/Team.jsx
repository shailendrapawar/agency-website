...
const Team = () => {
    ...
    return (
        <main className="container w-full h-auto  flex justify-center items-center relative">
            <section ref={ref} className={` ${isVisible==true?"opacity-100  slide-up":"opactiy-0"} inner-container h-full w-full  py-5 px-2 md:pt-20  max-w-250  flex flex-col  items-center justify-between gap-5 md:gap-8 relative  z-10 ${theme.name == "light" ? "backdrop-blur-3xl" : "backdrop-blur-[150px]"}`}>
                ...
                <div className=" gap-5 grid grid-cols-2 sm:grid-cols-3">
                    {
                        users?.map((user, i) => {
                            return <UserCard key={i} data={user} />
                        })
                    }
                </div>
