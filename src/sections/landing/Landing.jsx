  ...
  return (
    <main className="container w-full min-h-[calc(100vh-100px)]  flex justify-center items-center relative">
      ...
      <section className={`inner-container h-full w-full px-2 py-10 md:py-20  max-w-250  flex flex-col justify-center items-center gap-5 md:gap-8 relative z-10 ${theme.name == "light" ? "backdrop-blur-3xl" : "backdrop-blur-[80px]"}`}>
        ...
        <span className=" px-2 py-2 text-sm rounded-full gap-2 border-1 flex"
          style={{
        ...
        <img src={heroLandingImg}
          className="rounded-3xl sm:h-80 sm:w-160 md:h-100 w-200 object-cover">
        </img>

      </section>

      <aside className={`absolute h-50 w-50 z-1  right-[10%] sm:right-[15%] md:right-[25%]  top-[20%] rounded-full overflow-hidden ${theme.name == "light" ? "animate-pulse" : "animate-none"} delay-500 drop-shadow-blue-200 drop-shadow-2xl`}>
        <img src={blueWaterBg} />
      </aside>

      <aside className={`absolute h-80 w-80 drop-shadow-pink-500 drop-shadow-2xl z-1 bottom-0 left-[10%] sm:left-[15%] md:left-[25%] rounded-full overflow-hidden ${theme.name == "light" ? "animate-pulse" : "animate-none"} delay-700`}>
        <img src={pinkWaterBg} />
      </aside>
